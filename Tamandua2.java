...
public class Tamandua2 {
    public static void main(String[] args) throws java.io.IOException {
        InitPackage iPackage = Networking.getInit();
        int myID = iPackage.myID;
        GameMap gameMap = iPackage.map;
        
        ArrayList<Cell> tunnels = new ArrayList<Cell>();
        /**
        ...
        **/
        ...
        while (true) {
            ...
            //check the distance from the enemy, if it is less than 3  between, begin to focus the attack directly at them through a tunnel.
            for (Cell c : nBorderCells) {
                //test 3 ahead
                for (Direction d : Direction.CARDINALS) {
                    Site s = gameMap.getSite(gameMap.getLocation(gameMap.getLocation(new Location(c.x, c.y), d), d), d);
                    if (s.owner != myID && s.owner != 0) {
                        tunnels.add(c);
                    }
                }
                //test 2 ahead and 1 to either side
                //UPDATE Direction.java
                if (!tunnels[c.x][c.y]) {
                    for (Direction d : Direction.CARDINALS) {
                        Location l = gameMap.getLocation(gameMap.getLocation(new Location(c.x, c.y), d), d);
                        if (gameMap.getSite(l, turnRight(d, true)).owner != myID && gameMap.getSite(l, turnRight(d, true)).owner != 0) {
                            tunnels[c.x][c.y] = true;
                        } else if (gameMap.getSite(l, turnRight(d, false)).owner != myID && gameMap.getSite(l, turnRight(d, false)).owner != 0) {
                            tunnels[c.x][c.y] = true;
                        }
                    }
                }
                //testing 1 ahead and 2 to either side as well as 3 to either side has already been done by the two above structures.
            }
            
            //add weight to all cells within 2 moves of tunnels to move toward the tunnels.
            for (Cell c : cells) {
                for (Cell c2 : tunnels) {
                    if (getDistance(new Location(c.x, c.y), new Location(c2.x, c2.y)) <= 3) {
                        //c.weights[] //increase the weight of the direction toward the tunnel
                    }
                }
            }
